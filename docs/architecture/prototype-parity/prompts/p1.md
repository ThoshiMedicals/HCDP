...
- **Scope:** Shared shell + workbench primitives
- **Module:** MULTI
- **Primary route:** `shared-shell`
- **Design reference:** `derived-from-design-contract`
- **Screen IDs (sample/full list in registers):** see canonical-screen-register.json
- **Requirement IDs (non-exhaustive; full SoT in master register):** see master register
...
- Action/workflow IDs in scope: see workflow-action-register.json
...
- Owner note: Shared shell owns chrome only; modules own domain records
- Cross-module: contracts/events/projections only
...
- Persistence method today: N/A — foundation
...
- Visual QA against Decision A PNG / design-contract pattern for MULTI  
