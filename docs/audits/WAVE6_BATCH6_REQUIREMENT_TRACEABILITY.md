...
| # | Decision | Applied |
|---|---|---|
| 1 | Include explicit payroll-period locking | `explicitLockPayPeriod` + service-layer `period-lock-guard` (fail-closed missing period context; platform `*` export profiles; strict Gregorian effective dates) |
...
| R-6.5-01 | 6.5 | Period lock + mutation guard | omit-metadata; rates/rules/ext-id/intake; platform `*`; strict calendar dates | G / rem A–4 | proven |
