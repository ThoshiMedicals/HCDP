...
| `git diff b661b6c -- src scripts` | empty (0 lines) before start |
| Live target | `http://127.0.0.1:3501` (`next start`; do not kill; not `:3000`) |
| Evidence root | `…/owner-inspection-contradiction-correction-v1/agent-workstep-qa/` |
...
| ID | Check |
| --- | --- |
| PRE-01 | Confirm frozen SHA + empty `src`/`scripts` diff vs `b661b6c` |
...
| WF-TOPBAR | `/dashboard` | Dashboard / Action Inbox links; search; mobile Open menu |
