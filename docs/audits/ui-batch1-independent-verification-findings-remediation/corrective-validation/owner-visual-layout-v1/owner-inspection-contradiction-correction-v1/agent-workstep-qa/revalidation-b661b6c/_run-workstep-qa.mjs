/**
 * Work-Step QA — contradiction correction v1
 * Frozen app SHA b661b6c against :3501.
...
const APP_SHA = "b661b6c1e3debdd8ba4d1e71b55fe9f7052f90b1";
...
async function run() {
  // PREFLIGHT
  const diffLines = execSync("git diff b661b6c -- src scripts | wc -l", {
    cwd: WORKTREE,
    encoding: "utf8",
  }).trim();
  ...
  // ---------- PRE ----------
  {
    const wf = newWf("PREFLIGHT", "/", "sha+server");
    step(
      wf,
      "Empty git diff b661b6c -- src scripts",
      "0 lines",
      String(diffLines),
  ...
  // ---------- M04 ENGAGEMENTS ----------
  {
    ...
    await withPage({ width: 1440, height: 900 }, async (page) => {
      ...
      if ((await create.count()) > 0) {
        ...
        // fill required selects if needed
  ...
  // ---------- TOPBAR ----------
  {
    const wf = newWf("WF-TOPBAR", "/dashboard", "desktop+mobile");
    await withPage({ width: 1440, height: 900 }, async (page) => {
      await goto(page, "/dashboard", 1200);
