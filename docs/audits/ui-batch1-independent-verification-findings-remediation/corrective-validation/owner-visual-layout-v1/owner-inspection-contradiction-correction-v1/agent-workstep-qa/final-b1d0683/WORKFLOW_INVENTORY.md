...
| `git diff b1d0683 -- src scripts` | empty (0 lines) before start |
| Live target | `http://127.0.0.1:3501` (`next start`; do not kill; not `:3000`) |
| Evidence root | `…/owner-inspection-contradiction-correction-v1/agent-workstep-qa/final-b1d0683/` |
| Prior evidence (do not overwrite) | `…/agent-workstep-qa/revalidation-b661b6c/` |
...
| ID | Check |
| --- | --- |
| PRE-01 | Confirm frozen SHA + empty `src`/`scripts` diff vs `b1d0683` |
...
| WF-TOPBAR | `/dashboard` | Dashboard / Action Inbox; **Online** toggle; **Export** visible at xl+ (hidden below xl); search; mobile Open menu |
