/**
 * Visual QA Agent — complete capture at frozen SHA b661b6c against :3501
...
async function runChunk(chunk, workerId) {
  ...
  for (let i = 0; i < chunk.length; i++) {
    ...
    try {
      ...
      captures.push(rec);
      if ((i + 1) % 25 === 0 || job.critical) {
        console.log(`[w${workerId}] ${i + 1}/${chunk.length} ${name} hits=${probe.hitCount} hot=${probe.hotHits.length}`);
      }
    } catch (err) {
      captures.push({
        workerId,
