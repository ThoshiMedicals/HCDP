#!/usr/bin/env node
/**
 * Agent-regression sequential suite runner (evidence-only; does not modify app source).
 * Working directory must be the frozen-SHA worktree.
 */
...
const WT = process.env.HCDP_WT || process.cwd();
const EV =
  process.env.HCDP_EV ||
...
const FROZEN = "d68040688cbf76fb1f8715c27aa06ad6ff72242c";
const EXPECTED_HASH =
...
function parseLintTotals(text) {
  ...
  const m2 = text.match(/(\d+)\s+error/i);
  const m3 = text.match(/(\d+)\s+warning/i);
  return {
    errors: m2 ? Number(m2[1]) : null,
    warnings: m3 ? Number(m3[1]) : null,
  };
}
...
function restorePerf(snaps) {
  ...
  // Also git-checkout to guarantee clean tree for tracked perf/workflow/hash artifacts
  spawnSync(
    "git",
    [
      "checkout",
...
const suites = [
  ...
  { id: "27", name: "npm-build", cmd: ["npm", "run", "build"], kind: "build" },
  {
    id: "28",
    name: "hash-gate",
    cmd: ["node", "scripts/ui-batch1-iv-findings-remediation-hash-gate.mjs"],
    kind: "hash",
  },
];
...
for (const suite of suites) {
  ...
  if (suite.kind === "tsc") {
    ...
    if (errCount === 21) {
      verdict = "PASS";
      notes = "Accepted unchanged TypeScript debt: exactly 21 errors (baseline match).";
    } else {
      verdict = "FAIL";
    ...
    extras = { ...lint, expectedErrors: 2, expectedWarnings: 24 };
    if (lint.errors === 2 && lint.warnings === 24) {
      verdict = "PASS";
      notes = "Accepted unchanged lint debt: 2 errors + 24 warnings.";
    } else {
      verdict = "FAIL";
      notes = `Lint totals ${lint.errors}/${lint.warnings} != accepted 2/24.`;
    }
