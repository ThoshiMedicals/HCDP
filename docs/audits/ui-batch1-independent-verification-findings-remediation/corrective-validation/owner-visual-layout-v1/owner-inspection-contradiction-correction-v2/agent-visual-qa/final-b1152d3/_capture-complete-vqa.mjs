/**
 * Visual QA Agent — complete capture at frozen SHA b1152d3 against :3502
 * Evidence-only; does not modify src/ or scripts/.
 * Writes under agent-visual-qa/final-b1152d3/
 * (does not touch revalidation-b661b6c/ or final-b1d0683/).
 *
 * Close criteria (owner):
 * - 1024/1280/1440: Export/New Entry hidden until 2xl; Online in viewport; search width ≥160px
 * - 1536: Export, New Entry, Enterprise MFA, Online all fully in viewport; search usable
 * Must close: VQA-C-001, VQA-C-017, prior 110 module clipping surfaces
 */
...
const { chromium } = require("/tmp/hcdp-fix/c2a-vqa-3511/node_modules/playwright");

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const OUT = path.join(__dirname, "screenshots");
const CROPS = path.join(__dirname, "defect-crops");
const GEO = path.join(__dirname, "geometry");
const BASE = "http://127.0.0.1:3511";
const APP_SHA = "b1152d36d3f47c15277f85b3e990f5e1c94bddcb";
const WORKTREE = "/tmp/hcdp-fix/c2a-vqa-3511";
const SEARCH_MIN_WIDTH_MID = 160;
const SEARCH_MIN_WIDTH_2XL = 120; // "usable" at 1536 with Export/Enterprise present
...
async function probeMeaningfulControls(page) {
  return page.evaluate(() => {
    ...
    for (const el of nodes) {
      ...
      // Closed drawers/dialogs/mobile sidebar are not interactive surfaces (Correction 2).
      const closedSurface = el.closest('[role="dialog"][aria-hidden="true"], aside[aria-modal][aria-hidden="true"], [inert]');
      if (closedSurface) continue;
      // Closed <details> menus are not interactive (Correction 2A); skip like closed drawers.
      if (el.closest('details:not([open])')) continue;
      const sidebar = el.closest(".pulse-sidebar");
      if (sidebar) {
        const sbt = getComputedStyle(sidebar).transform || "";
        const sbm = sbt.match(/matrix\(([^)]+)\)/);
        const sbr = sidebar.getBoundingClientRect();
        const tx = sbm ? Number(sbm[1].split(",")[4]) : 0;
        if ((Number.isFinite(tx) && Math.abs(tx) > 8) || sbr.right <= 1) continue;
      }

      const overflowsViewportX = r.right > vpW + 1 || r.left < -1;
      const overflowsViewportY = r.bottom > vpH + 1 || r.top < -1;
      const entirelyOutsideY = r.bottom <= 1 || r.top >= vpH - 1;
      const belowViewportPageScroll = !overflowsViewportX && overflowsViewportY;
      // Vertical-only below/above fold is authorised page/sidebar scroll, not a defect.
      let outsideViewport = overflowsViewportX || (overflowsViewportY && !entirelyOutsideY && !belowViewportPageScroll);

      let clippedByAncestor = false;
      let ancestorInfo = null;
      let horizontalScrollEscape = false;
      ...
      while (n && n !== document.body) {
        ...
        if ((ox === "auto" || ox === "scroll") && scrollableX && overflowsViewportX) {
          horizontalScrollEscape = true;
        }
        const clips =
      ...
      // Authorised section-tab / table horizontal scroll — not a layout defect.
      if (horizontalScrollEscape) {
...
async function probeTopbarRibbon(page) {
  return page.evaluate(() => {
    ...
    // Breakpoint expectations (Tailwind defaults): 2xl=1536
    // Owner criteria at b1152d3: Export / New Entry / Enterprise all deferred until 2xl.
    const expectExportVisible = vpW >= 1536;
    const expectNewEntryVisible = vpW >= 1536;
    const expectEnterpriseVisible = vpW >= 1536;

    const exportBtn = findBtn(/^Export$/);
    const newEntry = findBtn(/^\+?\s*New Entry$/);
    const enterprise =
      findBtn(/^Enterprise MFA$/) ||
      findBtn(/Enterprise MFA/) ||
      findBtn(/Enterprise Sign-In/);
    ...
    const passNewEntry =
      expectNewEntryVisible
        ? NewEntry.present && !NewEntry.visuallyHidden && !NewEntry.clippedVp && NewEntry.right <= vpW + 1
        : !NewEntry.present || NewEntry.visuallyHidden || NewEntry.display === "none";
    const passEnterprise =
      expectEnterpriseVisible
        ? Enterprise.present &&
          !Enterprise.visuallyHidden &&
          !Enterprise.clippedVp &&
          !Enterprise.truncated &&
          Enterprise.right <= vpW + 1 &&
          (/Enterprise MFA|Enterprise Sign-In/.test(Enterprise.label || "") ||
            /Enterprise/.test(Enterprise.ariaLabel || ""))
    ...
    return {
      ...
      expectations: {
        ...
        onlineVisible: true,
        hideExportNewEntryUntil: "2xl/1536",
      },
      Export,
      NewEntry,
      Enterprise,
...
/** VQA-C-017 ribbon search geometry — width ≥160 at mid-desktop; usable at 1536 */
async function probeRibbonSearch(page) {
  return page.evaluate(
    ({ minMid, min2xl }) => {
      const vpW = window.innerWidth;
      const input =
        document.querySelector('input[aria-label="Search modules and sections"]') ||
        document.querySelector('input[type="search"]');
      if (!input) {
        return { found: false, vp: vpW, pass: false, reason: "missing-search-input" };
      }
      const r = input.getBoundingClientRect();
      const label = input.closest("label") || input.parentElement;
      const lr = label ? label.getBoundingClientRect() : null;
      const cs = getComputedStyle(input);
      const minRequired = vpW >= 1536 ? min2xl : vpW >= 1024 ? minMid : 0;
      const widthOk = minRequired === 0 ? true : r.width + 0.5 >= minRequired;
      const inViewport = r.right <= vpW + 1 && r.left >= -1;
      const placeholder = input.getAttribute("placeholder") || "";
      // crushed placeholder signal: content box too narrow to show more than ~4 chars
      const crushSignal = r.width > 0 && r.width < 80;
      return {
        found: true,
        vp: vpW,
        w: r.width,
        ...
        labelW: lr ? lr.width : null,
        overflow: cs.overflow,
        textOverflow: cs.textOverflow,
        placeholder,
        truncated: input.scrollWidth > input.clientWidth + 2,
        crushSignal,
        minRequired,
        widthOk,
        inViewport,
        pass: widthOk && inViewport && !crushSignal,
      };
    },
    { minMid: SEARCH_MIN_WIDTH_MID, min2xl: SEARCH_MIN_WIDTH_2XL }
  );
}
  ...
  for (let i = 0; i < chunk.length; i++) {
    ...
    try {
      ...
      if (job.topbarProbe || job.tier === "vqa-c001") {
        ...
        search = await probeRibbonSearch(page);
      }
      let focus = null;
      ...
      // VQA-C-017 proof crops — search zone at mid-desktop / 2xl
      if (search && job.route === "/dashboard" && [1024, 1280, 1440, 1536].includes(job.vp.w)) {
        const topH = Math.min(72, job.vp.h);
        const zoneName = `VQA-C-017-${job.mode}-${job.vp.id}-topbar-search-zone.png`;
        await page.screenshot({
          path: path.join(CROPS, zoneName),
          clip: { x: 0, y: 0, width: job.vp.w, height: topH },
        });
        crops.push(zoneName);
        if (search.found && Number.isFinite(search.x) && search.w > 0) {
          const crushName = `VQA-C-017-${job.mode}-${job.vp.id}-ribbon-search.png`;
          const sx = Math.max(0, Math.floor(search.x) - 24);
          const sw = Math.min(job.vp.w - sx, Math.ceil(search.w) + 80);
          await page.screenshot({
            path: path.join(CROPS, crushName),
            clip: { x: sx, y: 0, width: Math.max(sw, 40), height: topH },
          });
          crops.push(crushName);
        }
      }
      ...
      if ((i + 1) % 25 === 0 || job.critical || topbar) {
        ...
        const sp = search?.pass;
        console.log(
          `[w${workerId}] ${i + 1}/${chunk.length} ${name} hits=${probe.hitCount} hot=${probe.hotHits.length} topbarPass=${tv} searchPass=${sp} searchW=${search?.w}`
        );
      }
    } catch (err) {
      captures.push({
  ...
  JSON.stringify(
    {
      appSha: APP_SHA,
      criteria: {
        exportNewEntryEnterpriseVisibleAt: ">=1536 (2xl)",
        onlineAlwaysInViewport: true,
      },
...
// Aggregate VQA-C-017 ribbon search probes
...
const searchFails = searchFindings.filter((s) => s.pass === false);
fs.writeFileSync(
  path.join(GEO, "ribbon-search-probe.json"),
  JSON.stringify(
    {
      appSha: APP_SHA,
      criteria: {
        midDesktopMinWidthPx: SEARCH_MIN_WIDTH_MID,
        twoXlMinWidthPx: SEARCH_MIN_WIDTH_2XL,
        midDesktopWidths: [1024, 1280, 1440],
      },
      count: searchFindings.length,
      failCount: searchFails.length,
      findings: searchFindings,
      fails: searchFails,
      summaryByViewport: [1024, 1280, 1440, 1536].map((w) => {
        const rows = searchFindings.filter(
          (s) => s.vp === w || String(s.viewport || "").startsWith(`${w}x`)
        );
        const widths = rows.map((r) => r.w).filter((n) => Number.isFinite(n));
        return {
          w,
          samples: rows.length,
          minW: widths.length ? Math.min(...widths) : null,
          maxW: widths.length ? Math.max(...widths) : null,
          failCount: rows.filter((r) => r.pass === false).length,
        };
      }),
    },
    null,
...
const priorReprobe = results
  ...
  .map((r) => ({
    ...
    badHot: (r.probe.hotHits || []).filter(
      (h) =>
        !h.flags.horizontalScrollEscape &&
        !h.flags.belowViewportPageScroll &&
        ((h.flags.outsideViewport && h.flags.overflowsViewportX) ||
          (h.flags.clippedByAncestor && h.flags.overflowsViewportX) ||
          h.flags.overflowsViewportX)
    ),
  }));
fs.writeFileSync(
...
console.log(
  ...
  JSON.stringify({
    ...
    searchProbes: searchFindings.length,
    searchFails: searchFails.length,
    elapsedMs,
    out: OUT,
  })
);
