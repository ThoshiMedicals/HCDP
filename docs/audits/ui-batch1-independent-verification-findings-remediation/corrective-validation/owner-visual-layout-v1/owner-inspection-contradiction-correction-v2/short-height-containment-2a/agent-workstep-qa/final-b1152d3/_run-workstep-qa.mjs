/**
 * Work-Step QA — contradiction correction v2 (final-b1152d3)
 * Frozen app SHA b1152d3 against :3502.
 * Evidence-only under this folder. No src/ or scripts/ edits.
 * Does not overwrite revalidation-b661b6c/ or final-b1d0683/.
 */
...
async function run() {
  ...
  const results = {
    meta: {
      ...
      phase: "owner-inspection-contradiction-correction-v2",
  ...
  // ---------- PRE ----------
  {
    ...
    await withPage({ width: 1280, height: 800 }, async (page) => {
      ...
      step(wf, "GET /dashboard on :3502", "HTTP 2xx", `status=${res?.status()} url=${page.url()}`, res && res.ok() ? "PASS" : "FAIL");
      ...
      const cssHref = await page.evaluate(() => {
        const link =
          ...
          document.querySelector('link[rel="stylesheet"][href*="/_next/static/chunks/"]') ||
          document.querySelector('link[rel="stylesheet"][href*="/_next/static/"]');
...

  {
    ...
    await withPage({ width: 1440, height: 900 }, async (page) => {
      ...
      const filter = page.getByTestId("m06-history-filter");
      const beforeVal = (await filter.count()) > 0 ? await filter.inputValue() : "<missing>";
      step(wf, "History filter initial", "empty or existing value recorded", `value=${beforeVal}`, "PASS");
      if ((await filter.count()) === 0) {
        step(wf, "History filter present", "m06-history-filter exists", "missing", "FAIL");
      } else {
        await ensureOperable(wf, filter, "History filter");
        await filter.fill("demo-filter");
        const mid = await filter.inputValue();
        step(wf, "History filter set", "value=demo-filter", `value=${mid}`, mid === "demo-filter" ? "PASS" : "FAIL");
      }
      const clear = page.getByTestId("m06-history-clear-filter");
      await ensureOperable(wf, clear, "Clear filter");
      await clear.click();
      await waitReady(page, 600);
      const after = (await filter.count()) > 0 ? await filter.inputValue() : "<missing>";
      step(
        wf,
        "Clear filter (domain)",
        "History filter value empty after clear",
        `value=${after}`,
        after === "" ? "PASS" : "FAIL"
      );
  ...
  // ---------- TOPBAR ----------
  // Product intent at b1152d3: New Entry / Export / Enterprise MFA deferred to 2xl (≥1536).
  {
    const wf = newWf("WF-TOPBAR", "/dashboard", "desktop+2xl+xl+mobile");
    await withPage({ width: 1440, height: 900 }, async (page) => {
      await goto(page, "/dashboard", 1200);
      ...
      // Scope Export to top ribbon (page may also have an Export control).
      // Prefer 2xl:inline-flex class marker from Topbar.tsx (digit-leading class → attr contains).
      const topbarExport2xl = page
        .locator('button[class*="2xl:inline-flex"]')
      ...
      const topbarExport = page
        ...
        .filter({ hasText: /^Export$/ })
        .first();
      const exportLoc = (await topbarExport2xl.count()) > 0 ? topbarExport2xl : topbarExport;
      await exportLoc.waitFor({ state: "attached", timeout: 5000 }).catch(() => {});

      // Below 2xl (1440): topbar Export must remain deferred/hidden
      const exportAt1440Display = await exportLoc
        .evaluate((el) => getComputedStyle(el).display)
        .catch(() => "missing");
      const exportAt1440Vis = await exportLoc.isVisible().catch(() => false);
      step(
        wf,
        "Topbar Export hidden below 2xl (1440)",
        "display=none (topbar only; deferred to 2xl)",
        `visible=${exportAt1440Vis} display=${exportAt1440Display}`,
        exportAt1440Display === "none" || !exportAt1440Vis ? "PASS" : "FAIL"
      );
      if (!(exportAt1440Display === "none" || !exportAt1440Vis)) {
        addFinding(
          "WQA-TOPBAR-EXPORT-BELOW-2XL",
          "WF-TOPBAR",
          "major",
          "Topbar Export visible below 2xl (should be deferred)",
          JSON.stringify({ exportAt1440Vis, exportAt1440Display })
        );
      }

      // At 2xl (1536): Export visible + operable; Enterprise MFA short label present
      ...
      if (
        ...
        addFinding(
          "WQA-TOPBAR-EXPORT-2XL",
          "WF-TOPBAR",
          "major",
          "Topbar Export not visible at 2xl+",
          JSON.stringify({ exportVis, exportDisplay, exportProbe })
        );
      }
      const entMfa = page.getByRole("button", { name: /Enterprise Sign-In · MFA|Enterprise MFA/i }).first();
      const entText = ((await entMfa.innerText().catch(() => "")) || "").replace(/\s+/g, " ").trim();
      const entVis = await entMfa.isVisible().catch(() => false);
      step(
        wf,
        "Enterprise MFA short label at 2xl",
        "Visible button text is 'Enterprise MFA'",
        `visible=${entVis} text=${JSON.stringify(entText)}`,
        entVis && /^Enterprise MFA$/i.test(entText) ? "PASS" : "FAIL"
      );
      if (!(entVis && /^Enterprise MFA$/i.test(entText))) {
        addFinding(
          "WQA-TOPBAR-ENT-MFA-LABEL",
          "WF-TOPBAR",
          "major",
          "Enterprise MFA short label missing/incorrect at 2xl",
          `visible=${entVis} text=${JSON.stringify(entText)}`
        );
      }

      // Negative check: topbar Export hidden below 2xl (1024) — ignore in-page Export
      ...
      step(
        wf,
        "Topbar Export hidden below 2xl (1024)",
      ...
      await page.setViewportSize({ width: 1536, height: 900 });
      await waitReady(page, 400);
      wf.screenshots.push(await shot(page, "WF-TOPBAR-1536"));
    });
    results.workflows.push(finalize(wf));
  }
